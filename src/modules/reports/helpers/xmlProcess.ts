...
export async function checkRFCExist(siren: string): Promise<{ total: number }> {
  ...
  const row = await queryOne<{ total: number }>(
    `SELECT COUNT(*) AS total FROM llx_societe WHERE siren='${siren}' AND fournisseur=1 LIMIT 1`,
    [siren]
  );
...
export async function checkExist(
  ...
  const soc = await queryOne<{ rowid: number }>(
    `SELECT rowid FROM llx_societe WHERE siren='${rfc}' LIMIT 1`,
    [rfc]
  );
  ...
  const row = await queryOne<{ total: number }>(
    `SELECT COUNT(*) as total
     ...
     WHERE REPLACE(ref_fourn,'|','')='${ref}' AND fk_soc='${soc.rowid}'`,
    [ref, soc.rowid]
  );
  return { total: row?.total ?? 0 };
}
...
export async function checkLog(
  ...
  const row = await queryOne<{ total: number }>(
    `SELECT COUNT(*) AS total
     ...
     WHERE provider='${provider}' AND invoice_number='${invoice_number}'`,
...
export async function createLog(
  ...
  await query(
    `INSERT INTO llx_process_invoice_xml
     ...
     VALUES ('${provider}', '${invoice_number}', '${invoice_date}', '${total_products}', NOW())`,
...
export async function createDraft(
  ...
  const soc = await queryOne<{ fk_soc: number }>(
    `SELECT fp.fk_soc
     ...
     WHERE REPLACE(fp.ref_fourn,'|','')='${ref}' AND s.siren='${rfc}' AND fp.fk_soc IS NOT NULL
  ...
  await query(
    `INSERT INTO llx_commande_fournisseur
     ...
     VALUES (1, '${invoice}', '${soc.fk_soc}', NOW(), NOW(), '${FK_USER_AUTHOR}', '${tva}', '${total_ht}', '${total}', 'muscadet', 2, 67)`,
  ...
  const last = await queryOne<{ rowid: number }>(
    `SELECT rowid FROM llx_commande_fournisseur
     WHERE fk_user_author='${FK_USER_AUTHOR}'
  ...
  // Actualizar ref
  await query(
    `UPDATE llx_commande_fournisseur SET ref='(PROV${last.rowid})' WHERE rowid='${last.rowid}' LIMIT 1`,
    [last.rowid]
  );
  return true;
}
...
export async function createDraftDet(
  ...
  const prod = await queryOne<{
    ...
    `SELECT pf.fk_product, pf.fk_soc, pf.ref_fourn, p.ref
     ...
     WHERE REPLACE(pf.ref_fourn,'|','')='${ref}'
  ...
  const extr = await queryOne<{ contenido: number | null }>(
    `SELECT contenido FROM llx_product_extrafields WHERE fk_object='${prod.fk_product}' LIMIT 1`,
    [prod.fk_product]
  );
  ...
  await query(
    `INSERT INTO llx_commande_fournisseurdet
     ...
     VALUES ('${last.rowid}', '${prod.fk_product}', '${ref}', 16, '${qty}', '${remise_percent}', '${unitNoIVA}', '${subtotal}', '${iva}', '${totalconiva}')`,
    [
      last.rowid,
      prod.fk_product,
      ref,
